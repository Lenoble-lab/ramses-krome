...
!-------------------------------------------------------------------------
  ...
  real(kind=8)::nH, T2, ekk, err, emag, x, mu, Zsolar
  real(kind=8),dimension(nIons)::phI_rates   ! Photoionization rates [s-1]
  real(kind=8),dimension(7)::nSpec           !          Species abundances
#if NENER>0
  integer::irad
#endif
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  real(rtdp),dimension(nrtvartrace)::rtvars
  real(kind=8),dimension(nIons)::xion
  integer::ip, iI, idim
  real(dp)::scale_nH, scale_T2, scale_l, scale_d, scale_t, scale_v
  real(dp)::scale_Np, scale_Fp
  real(kind=8)::nH, T2, ekk, err, emag, mu, Zsolar, ss_factor
  real(kind=8),dimension(nIons)::phI_rates       ! Photoionization rates [s-1]
  real(kind=8),dimension(7)::nSpec               !          Species abundances
#if NENER>0
  integer::irad
