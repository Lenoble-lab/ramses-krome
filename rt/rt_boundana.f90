...
subroutine rt_boundana(x,u,dx,ibound,ncell)
  use amr_parameters, ONLY: dp,rtdp,ndim,nvector
  use rt_parameters, ONLY: rt_boundary_var, nrtvartrace
  implicit none
  integer ::ibound                              ! Index of boundary region
  integer ::ncell                               !   Number of active cells
  real(dp)::dx                                  !                Cell size
  real(rtdp),dimension(1:nvector,1:nrtvartrace)::u !     Conservative vars
