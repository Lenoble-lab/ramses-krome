...
!-------------------------------------------------------------------------
  ...
  use amr_parameters,only: improved_h2_cooling
  implicit none
  real(kind=8)::T, ne, T3
  ...
  real(kind=8),save::cr_H2HI,cr_H2H2,cr_H2HI_prime,cr_H2H2_prime,rateH2,rateH
...
!-------------------------------------------------------------------------
  ...
  if(isH2) then
     ...
     if (improved_h2_cooling) then  ! Cooling from Hallenbach McKee (1979)
     rateH    = LvrH(T,nN(ixHII)) 
     cr_H2HI       = rateH  &
                   * nN(ixHI) * nN(ixHII) ! nH2 * nHI

     cr_H2HI_prime = (LvrH(T*1.0001,nN(ixHII))-rateH)/(0.0001*T) * nN(ixHI)*nN(ixHII) * T * log(10d0)
	 rateH2   = LvrH2(T,nN(ixHI))
     cr_H2H2       = rateH2  &
                   * nN(ixHI)**2 ! nH2 * nH2
     cr_H2H2_prime = (LvrH2(T*1.0001,nN(ixHI))-rateH2)/(0.0001*T) * nN(ixHI)**2 * T * log(10d0)
     else  						    ! Cooling from Nickerson Teyssier (2018) (Approximation to low density)
...
ELEMENTAL FUNCTION Lr_LTE(T)
  !-------------------------------------------------------------------------
  implicit none
  real(kind=8),intent(in)::T
  real(kind=8)::Lr_LTE, T3
  !----------------------------------------------------------
  T3 = T/1d3
  Lr_LTE = (9.5d-22*T3**3.76/(1.0+0.12*T3**2.1)*exp(-(0.13/T3)**3))+3.0d-24*exp(-0.51/T3)

END FUNCTION Lr_LTE

ELEMENTAL FUNCTION Lv_LTE(T)
  !-------------------------------------------------------------------------
  implicit none
  real(kind=8),intent(in)::T
  real(kind=8)::Lv_LTE, T3
  !----------------------------------------------------------
  T3 = T/1d3
  Lv_LTE = (6.7d-19*exp(-5.86/T3)+1.6d-18*exp(-11.7/T3))
...
ELEMENTAL FUNCTION LrH2_0(T)
  !-------------------------------------------------------------------------
  implicit none
  real(kind=8),intent(in)::T
  real(kind=8)::LrH2_0
  !----------------------------------------------------------
  LrH2_0 = (1.25*exp(-1.70d2/T)*2.35d-14)*gamma_h2(T,2d0) + (1.75*exp(-5.05d2/T)*6.97d-14)*gamma_h2(T,3d0)

END FUNCTION LrH2_0

ELEMENTAL FUNCTION LvH2_0(T)
  !-------------------------------------------------------------------------
  implicit none
  real(kind=8),intent(in)::T
  real(kind=8)::LvH2_0
  !----------------------------------------------------------
  LvH2_0 = exp(-5860.0/T)*8.09d-13*1.4d-12*sqrt(T)*exp(-1.2d4/(T+1.2d3))
...
ELEMENTAL FUNCTION LrH_0(T)
  !-------------------------------------------------------------------------
  implicit none
  real(kind=8),intent(in)::T
  real(kind=8)::LrH_0
  !----------------------------------------------------------
  LrH_0 = (1.25*exp(-1.70d2/T)*2.35d-14)*gamma_hi(T,2d0) + (1.75*exp(-5.05d2/T)*6.97d-14)*gamma_hi(T,3d0)

END FUNCTION LrH_0

ELEMENTAL FUNCTION LvH_0(T)
  !-------------------------------------------------------------------------
  implicit none
  real(kind=8),intent(in)::T
  real(kind=8)::LvH_0
  !----------------------------------------------------------
  LvH_0 = exp(-5860.0/T)*8.09d-13*1.0d-12*sqrt(T)*exp(-1.0d3/(T)) + exp(-5860.0*2/T)*2*8.09d-13*1.6d-12*sqrt(T)*exp(-(4d2/(T)**2))
...
FUNCTION LvrH(T,ixHII)
  !-------------------------------------------------------------------------
  implicit none
  real(kind=8),intent(in)::T,ixHII
  real(kind=8)::LvrH
  !----------------------------------------------------------*
  
  LvrH = (Lr_LTE(T)/ixHII)/(1+(Lr_LTE(T)/ixHII)/LrH_0(T)) + (Lv_LTE(T)/ixHII)/(1+(Lv_LTE(T)/ixHII)/LvH_0(T))

END FUNCTION LvrH

FUNCTION LvrH2(T,ixHI)
  !-------------------------------------------------------------------------
  implicit none
  real(kind=8),intent(in)::T,ixHI
  real(kind=8)::LvrH2
  !----------------------------------------------------------
  LvrH2 = (Lr_LTE(T)/ixHI)/(1+(Lr_LTE(T)/ixHI)/LrH2_0(T))  + (Lv_LTE(T)/ixHI)/(1+(Lv_LTE(T)/ixHI)/LvH2_0(T))
