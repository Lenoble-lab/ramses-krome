...
!-------------------------------------------------------------------------
  ...
  real(rtdp),dimension(:),allocatable::xx
  real(dp)::gamma2
  ...
  if (ntraceGroups.gt.0) then
     ...
     rtuold(1:ncell,nrtvar+1:nrtvartrace) = 1.0/ntraceGroups
     rtunew(1:ncell,nrtvar+1:nrtvartrace) = 1.0/ntraceGroups
  end if
#endif
  ...
  read(ilun)gamma2
  if(nrtvar2.gt.nrtvartrace .and. myid==1)then ! Not ok to drop RT variables 
