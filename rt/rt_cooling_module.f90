...
contains
  ...
  !-----------------------------------------------------------------------
    ...
    if(c_switch(icell) .and. cooling .and. .not. rt_T_rad) then
       ...
       if(Zsolar(icell) .gt. 0d0) then
       		print*,'NONZERO METALLICIY'
       ...
       !print*,'Cooling rate',X_nHkb*Crate
       !print*,'cooling in sec',300/(X_nHkb*Crate)

       dRate = -X_nHkb*(dCdT2 + Zsolar(icell)*metal_prime)     ! dRate/dT2
       !print*,'dRate*ddt',dRate*ddt(icell)
       !print*,'Ion fraction',nN
       ! 1st order dt constr
