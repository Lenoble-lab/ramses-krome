...
subroutine init_flow_fine(ilevel)
  ...
  real(dp)::dx,rr,vx,vy,vz,ek,ei,pp,xx1,xx2,xx3,dx_loc,scale,xval,cosmo_ratio_D_H
  real(dp),dimension(1:3)::skip_loc
  real(dp),dimension(1:twotondim,1:3)::xc
  real(dp),dimension(1:nvector)       ,save::vv
  real(dp),dimension(1:nvector,1:ndim),save::xx
  real(dp),dimension(1:nvector,1:nvar),save::uu
  real*8 :: krome_x(krome_nmols),krome_n(krome_nmols), krome_mass(krome_nmols)
  real*8 :: rhogas
  ...
  ! krome initial condition in number densities (1/cm3)
  cosmo_ratio_D_H = 2.6e-5 ! ratio D/H

  krome_n(1)  = 2.11214d-4          !E
  krome_n(2)  = 1.3599d-11          !H-
  krome_n(3)  = 0.92378             !H
  krome_n(4)  = 1.2684567d-06	      !H2
  krome_n(5)  = 4.298503d-05	      !D
  krome_n(6)  = 2.3494293d-10	      !HD
  krome_n(7)  = 0.076               !HE
  krome_n(8)  = 2.11222d-4          !H+
  krome_n(9)  = 1.0417831d-13       !H2+
  krome_n(10) = 8.4554474d-09	      !D+
  krome_n(11) = 1.2969047d-15       !HEH+

  krome_mass = krome_get_mass()
  rhogas =  sum(krome_mass(1:krome_nmols) * krome_n)
  krome_x = krome_n2x(krome_n, rhogas) !convert number density to mass fraction
  ...
  if(ok_file)then
    ...
    do ivar=1,nvar
      ...
      if(krome_chem) then
        ...
        if(ivar==ndim+2+1) init_array = krome_x(1)    !E
        if(ivar==ndim+2+2) init_array = krome_x(2)    !H-
        if(ivar==ndim+2+3) init_array = krome_x(3)    !H
        if(ivar==ndim+2+4) init_array = krome_x(4)    !H2
        if(ivar==ndim+2+5) init_array = krome_x(5)    !D
        if(ivar==ndim+2+6) init_array = krome_x(6)    !HD
        if(ivar==ndim+2+7) init_array = krome_x(7)    !HE
        if(ivar==ndim+2+8) init_array = krome_x(8)    !H+
        if(ivar==ndim+2+9) init_array = krome_x(9)    !H2+
        if(ivar==ndim+2+10) init_array = krome_x(10)  !D+
        if(ivar==ndim+2+11) init_array = krome_x(11)  !HEH+
