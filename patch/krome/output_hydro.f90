...
real*8::unoneq(krome_nmols), T_eos
real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
real(kind=8),allocatable, dimension(:):: pressure_output, mu_output, temp_output
real(kind=8)::dtcool,nISM,nCOM,damp_factor,cooling_switch,t_blast
real(dp)::polytropic_constant=1

!set nISM as in cooling_fine
nISM = n_star

! Conversion factor from user units to cgs units (to convert the temperature)
...
do ilevel = 1, nlevelmax
    do ibound = 1, nboundary+ncpu
        ...
        if (ncache > 0) then
            allocate(ind_grid(1:ncache), xdp(1:ncache), pressure_output(1:ncache), mu_output(1:ncache), temp_output(1:ncache))
            ...
            !Write mu
            do i = 1, ncache
                !get krome abundancies
                do ivar = ndim+3+nener, nvar
                    unoneq(ivar - ndim - 3 - nener+1) = uold(ind_grid(i)+iskip,ivar)/max(uold(ind_grid(i)+iskip, 1), smallr)
                end do
                !get the mean molecular weight
                mu_output(i) = krome_get_mu_x(unoneq(:))
            ...
            ! Write Tgas
            do i = 1, ncache
                xdp(i) = pressure_output(i)/max(uold(ind_grid(i)+iskip, 1), smallr) * mu_output(i) * scale_T2            
                field_name = 'Tgas'
                temp_output(i) = xdp(i)
            end do
            call generic_dump(field_name, info_var_count, xdp, unit_out, dump_info_flag, unit_info)
            ...
            do ivar = ndim+3+nener, nvar
                ...
                if (metal .and. imetal == ivar) then
                    ...
                    write(field_name, *) names(ivar - ndim - 3 - nener+1)
                end if
                call generic_dump(field_name, info_var_count, xdp, unit_out, dump_info_flag, unit_info)
            end do
#endif
            ...
            deallocate(ind_grid, xdp, pressure_output, mu_output, temp_output)
