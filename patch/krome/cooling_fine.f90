  ...
  do ind=1,twotondim
    ...
    if(krome_chem)then
      ...
      do i=1,nleaf
        ...
        unoneq(4) = uold(ind_leaf(i),ndim+2+4) !H2
        unoneq(5) = uold(ind_leaf(i),ndim+2+5) !D
        unoneq(6) = uold(ind_leaf(i),ndim+2+6) !HD
        unoneq(7) = uold(ind_leaf(i),ndim+2+7) !HE
        unoneq(8) = uold(ind_leaf(i),ndim+2+8) !H+
        unoneq(9) = uold(ind_leaf(i),ndim+2+9) !H2+
        unoneq(10) = uold(ind_leaf(i),ndim+2+10) !D+
        unoneq(11) = uold(ind_leaf(i),ndim+2+11) !HEH+

        ! KROME: from code units to 1/cm3 for KROME
        unoneq(1) = unoneq(1)*scale_d/9.10938188e-28 !E
        unoneq(2) = unoneq(2)*scale_d/1.6744434563759998e-24 !H-
        unoneq(3) = unoneq(3)*scale_d/1.673532518188e-24 !H
        unoneq(4) = unoneq(4)*scale_d/3.347065036376e-24 !H2
        unoneq(5) = unoneq(5)*scale_d/3.346032518188e-24 !D
        unoneq(6) = unoneq(6)*scale_d/5.0195650363760004e-24 !HD
        unoneq(7) = unoneq(7)*scale_d/6.692065036376e-24 !HE
        unoneq(8) = unoneq(8)*scale_d/1.67262158e-24 !H+
        unoneq(9) = unoneq(9)*scale_d/3.346154098188e-24 !H2+
        unoneq(10) = unoneq(10)*scale_d/3.34512158e-24 !D+
        unoneq(11) = unoneq(11)*scale_d/8.364686616375999e-24 !HEH+
        ...
        !push redshift to Krome
        call krome_set_zredshift(1/aexp - 1)
        call krome_set_Tcmb(Tcmb0*1/aexp)
        call krome_set_user_Trad(Tcmb0*1/aexp)
        ...
        unoneq(2) = unoneq(2)*1.6744434563759998e-24*iscale_d !H-
        unoneq(3) = unoneq(3)*1.673532518188e-24*iscale_d !H
        unoneq(4) = unoneq(4)*3.347065036376e-24*iscale_d !H2
        unoneq(5) = unoneq(5)*3.346032518188e-24*iscale_d !D
        unoneq(6) = unoneq(6)*5.0195650363760004e-24*iscale_d !HD
        unoneq(7) = unoneq(7)*6.692065036376e-24*iscale_d !HE
        unoneq(8) = unoneq(8)*1.67262158e-24*iscale_d !H+
        unoneq(9) = unoneq(9)*3.346154098188e-24*iscale_d !H2+
        unoneq(10) = unoneq(10)*3.34512158e-24*iscale_d !D+
        unoneq(11) = unoneq(11)*8.364686616375999e-24*iscale_d !HEH+
        ...
        uold(ind_leaf(i),ndim+2+10) = unoneq(10)
        uold(ind_leaf(i),ndim+2+11) = unoneq(11)
    ...
    else if(cooling .or. neq_chem.or.krome_chem)then
      do i=1,nleaf
