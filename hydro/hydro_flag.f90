...
#endif
#if NENER>0
       & sum(err_grad_prad(1:NENER)).gt.-nener .and.&
#endif
#if NVAR>8+NENER
       &sum(err_grad_var(1:NVAR-8-NENER)).gt.-(nvar-8-nener) .and.&
#endif
...
subroutine jeans_length_refine(ind_cell,ok,ncell,ilevel)
  ...
  do i=1,ncell
     ...
     ! prevent Jeans refinement everywhere in cosmo sims
     if (cosmo.and.dens.le.(del_star * omega_b / omega_m)) then
        cycle
     endif 
