subroutine courant_fine(ilevel)
  ...
  real(kind=8)::mass_loc,ekin_loc,eint_loc,dt_loc, rho_max_loc
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        !compute max rho
        do i=1,nleaf
           rho_max_loc = max(uu(i,1), rho_max_loc)
        end do
#if NENER>0
        do ivar=1,nener
           do i=1,nleaf
  ...
  call MPI_ALLREDUCE(dt_loc,dt_all,1,MPI_DOUBLE_PRECISION,MPI_MIN,&
       &MPI_COMM_WORLD,info)
   call MPI_ALLREDUCE(rho_max_loc,rho_max_tot,1,MPI_DOUBLE_PRECISION,MPI_MAX,&
       &MPI_COMM_WORLD,info)
