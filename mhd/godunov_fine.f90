...
subroutine scale_cosmomag(ind_cell,exp_scale)
  ...
  ! Do the same for cosmic rays
#if NCR>0
  ! Expand cosmic rays energy density with anti-drag Hubble term
  ! and update their contribution to cell energy
  do igroup=1,ncr
     unew(ind_cell,5) = unew(ind_cell,5) - unew(ind_cell,inener-1+igroup)
     unew(ind_cell, inener-1+igroup) = &
          unew(ind_cell, inener-1+igroup) &
          *( 1.0d0 + (5.-3.*gamma_rad(1)) * (exp_scale**2-1.0) )
     unew(ind_cell,5) = unew(ind_cell,5) + unew(ind_cell,inener-1+igroup)
  end do
#endif
