...
subroutine init_flow_fine(ilevel)
  ...
  if(ok_file)then
     ...
     do ivar=1,nvar+3
        ...
        if(ncache>0)then
           if(cosmo)then
              ...
              if(ivar==9)init_array=(1.0+init_array)*T2_start/scale_T2/(1d0+mu_electron/mu_ion)
#if NCR>0
              if(ivar==10)init_array=(1.0+init_array)*T2_start*1d-10
#endif
#else
              if(ivar==5)init_array=(1.0+init_array)*T2_start/scale_T2
#if NCR>0
              if(ivar==9)init_array=(1.0+init_array)*T2_start/scale_T2*1d-10
#endif
#endif
           endif
           ! Loop over cells
...
#if NENER>0
                 do ivar=1,nener
                    uold(ind_cell(i),8+ivar)=uold(ind_cell(i),1)*uold(ind_cell(i),8+ivar)
                 enddo
#endif
              end do
           end do
     ...
     ! Initialize with random B-field
     if(napot>0)then
        ! Loop over grids by vector sweeps
        do igrid=1,ncache,nvector
           ngrid=MIN(nvector,ncache-igrid+1)
           ...
           do ind=1,twotondim
              ...
              ! Scatter left B values
              do ivar=6,8
                 do i=1,ngrid
                    uold(ind_cell(i),ivar)=uu(i,ivar)
                 end do
              end do
              ! Scatter right B values
